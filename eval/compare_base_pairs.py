...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare base vs math/coder parameter differences.")
    parser.add_argument("--base", type=Path, required=True, help="Path to base model.safetensors")
    parser.add_argument("--coder", type=Path, required=True, help="Path to coder model.safetensors")
    parser.add_argument("--math", type=Path, required=True, help="Path to math model.safetensors")
    parser.add_argument("--only-mlp", action="store_true", help="Only compare MLP-related tensors")
    args = parser.parse_args()

    pattern = "mlp" if args.only_mlp else None

    coder_stats = compare_models(args.base, args.coder, pattern)
    math_stats = compare_models(args.base, args.math, pattern)

    print_stats("base vs coder", coder_stats)
    print_stats("base vs math", math_stats)
    return 0
